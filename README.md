[![Build Status](https://git.vauxoo.com/vauxoo/elecaustro/badges/14.0/pipeline.svg)](https://git.vauxoo.com/vauxoo/elecaustro/pipelines)
[![Coverage Report](https://git.vauxoo.com/vauxoo/elecaustro/badges/14.0/coverage.svg)](https://coverage.vauxoo.com/14-0-elecaustro)
[![Runbot Status](http://runbot.vauxoo.com/runbot/badge/109/14.0.svg)](http://runbot.vauxoo.com/runbot/repo/git-git-vauxoo-com-vauxoo-elecaustro-git-109)



Modules for elecaustro
===
...
1. Install travis2docker in order to a create local environment (docker
   container) copy of last changes on the elecaustro repository:
    ...
    of elecaustro for make functional tests or even for do new developments:
...
3. Now we can use the t2d command. This will be accomplish by using the next
   command line:
    ...
    ```bash
    $ t2d git@git.vauxoo.com:vauxoo/elecaustro.git 14.0
    ```

    The result after run this t2d script will be a log that have a list of
    local paths, in elecaustro case, will have a list of 2 paths

    ```bash
    ['${HOME}/t2d/script/git_git.vauxoo.com_vauxoo_elecaustro.git/14.0/3.8/env_1_job_1',
     '${HOME}/t2d/script/git_git.vauxoo.com_vauxoo_elecaustro.git/14.0/3.8/env_2_job_1']
    ```
...
4. The next step is to run the last mentioned scripts, we highly recommend to
   add the next extra arguments.
    ...
    ```bash
    ${HOME}/t2d/script/git_git.vauxoo.com_vauxoo_elecaustro.git/14.0/3.8/env_2_job_1/10-build.sh --no-cache
    ${HOME}/t2d/script/git_git.vauxoo.com_vauxoo_elecaustro.git/14.0/3.8/env_2_job_1/20-run.sh --entrypoint=bash
    ```
...
5. After the last script is finish you will notice that your prompt has change,
   ...
   like this:
    ...
    ```bash
    [odoo@a458896bf8f5]~/build/vauxoo/elecaustro$
    (14.0) $
    ```
    ...
    elecaustro:
        ...
        - main repository will be ALWAYS inside /root/build/vauxoo/elecaustro
